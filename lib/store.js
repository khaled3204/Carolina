...
function blobBaseUrl() {
  ...
  // The store's URL subdomain drops the "store_" prefix and is lowercase,
  // e.g. store_tYehKY1jJBZQGuHd -> tyehky1jjbzqguhd.private.blob.vercel-storage.com
  const sub = storeId.replace(/^store_/i, '').toLowerCase();
  return `https://${sub}.private.blob.vercel-storage.com`;
}
...
async function readFromBlob() {
  ...
  // Private blobs at a known store follow a predictable URL. Reading it directly
  // (with cache=0 to bypass the CDN) avoids an extra list() round trip that can
  // itself lag behind a very recent overwrite.
  const base = blobBaseUrl();
  if (base) {
    try {
      const direct = await fetch(`${base}/${BLOB_PATHNAME}?cache=0`, {
        cache: 'no-store',
  ...
  try {
    ...
    const res = await fetch(freshUrl, { cache: 'no-store', headers: { Authorization: `Bearer ${token}` } });
  ...
  try {
    const res = await fetch(`https://blob.vercel-storage.com/${BLOB_PATHNAME}`, {
      ...
      headers: {
        ...
        // Without this, Vercel still appends a random suffix to the filename
        // on every save despite allow-overwrite — meaning every save silently
        // created a brand-new, differently-named file instead of updating the
        // same one, which is why nothing could ever be found/read back again.
