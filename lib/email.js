...
async function sendVerificationCodeEmail({ to, code }) {
  ...
  try {
    await transporter.sendMail({
      from: `"Carolina" <${user}>`,
      to,
      subject: `${code} is your Carolina verification code`,
      text: [
        `Your Carolina verification code is: ${code}`,
        '',
      ...
      html: `
        <div style="font-family: Georgia, serif; color: #1a1a1a; max-width: 480px; margin: 0 auto;">
          ...
          <p style="font-size: 32px; letter-spacing: 8px; font-weight: 700; background:#f5f0e6; padding: 16px 20px; text-align:center;">${code}</p>
...
async function sendOrderConfirmationEmail({ to, order }) {
  ...
  const itemsText = (order.items || [])
    .map((i) => `  ${i.qty} x ${i.name} (${i.color}/${i.size}) — $${i.lineTotal.toFixed(2)}`)
    .join('\n');
  ...
  const itemsHtml = (order.items || [])
    .map(
      ...
      <tr>
        ...
        <td style="padding:8px 0;text-align:right;">$${i.lineTotal.toFixed(2)}</td>
      </tr>`
    )
    .join('');
      ...
      html: `
        <div style="font-family: Georgia, serif; color: #1a1a1a; max-width: 520px; margin: 0 auto;">
          ...
          <table style="width:100%;border-collapse:collapse;margin-top:16px;">
            ${itemsHtml}
            <tr><td style="padding:8px 0;border-top:1px solid #ddd;">Subtotal</td><td style="padding:8px 0;border-top:1px solid #ddd;text-align:right;">$${order.subtotal.toFixed(2)}</td></tr>
            <tr><td style="padding:8px 0;">Shipping</td><td style="padding:8px 0;text-align:right;">$${order.shippingFee.toFixed(2)}</td></tr>
            <tr><td style="padding:8px 0;font-weight:700;">Total</td><td style="padding:8px 0;text-align:right;font-weight:700;">$${order.total.toFixed(2)}</td></tr>
          </table>
